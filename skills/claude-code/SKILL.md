...
## Description
This skill enables OpenClaw to spawn Claude Code as an isolated worker agent for code generation, implementation, and file manipulation tasks. Claude Code serves as the primary worker agent in the DevFlow Auto-Dev system, executing specific coding tasks assigned by the orchestrator and returning results through file-based state management.
...
CLAUDE_MODEL=claude-sonnet-4-20250514  # Optional: Model selection
CLAUDE_MAX_TOKENS=8192            # Optional: Max output tokens
```
...
├── state/
│   ├── workers/
│   │   └── claude-code-{id}.json   # Worker state files
│   └── output/
│       └── claude-code-{id}.json   # Worker output files
└── context/
    └── tasks/
...
```json5
{
  ...
  "params": {
    ...
    "output_file": "~/.openclaw/state/output/claude-code-001.json",
    "timeout": 300000,
    "isolation": true
  }
}
...
OpenClaw spawns Claude Code workers using the `sessions_spawn` protocol:
...
```bash
# Direct Claude Code invocation for testing
claude --print "Implement the feature described in spec.md" \
...
2. **Execution Phase**
   ...
   - Executes Claude Code with appropriate prompts
...
4. **HALT Protocol** (when blocked)
   ...
   actions:
     - write_state_to_file: ~/.openclaw/state/workers/claude-code-{id}.json
...
Spawned Claude Code workers operate under these constraints:
...
```json
{
  ...
  "state_file": "~/.openclaw/state/workers/claude-code-001.json"
}
```
...
```bash
# Recovery command
claude --resume ~/.openclaw/state/workers/claude-code-001-checkpoint.json
```
...
claude-code-worker \
  ...
  --output ~/.openclaw/state/output/claude-code-auth.json \
  --timeout 300000
```
...
5. **Audit Trail**: All actions logged to `~/.openclaw/logs/claude-code/`
